...
async function main() {
  ...
  const LpToken = await hre.ethers.getContractFactory("LpTokenV3");
  const lp = await LpToken.deploy("Snow usdc/fusdc/fusdt/fdai", "usdc/fusdc/fusdt/fdai");

  const lpContract = await lp.deployed();
  ...
  const StableSwap = await hre.ethers.getContractFactory("StableSwapV3");
  ...
  const stableSwapContract = await stableSwap.deployed();
